...
#  include "lib_clock.h"
#  include "lib_lcd.h"
#  define t_print u_print
#  define t_printInt(VALUE) u_printInt(VALUE)
#endif
...
*/
	index = __builtin_brownie32_CLAMPI(index);
	...
	** in adpc.h -> adpcmDataStereo_MINI.h
m_coder.
	*/
/**
	vpdiff = step >> 3;
...
**/
	unsigned int arg = ((step<<16) & 0xFFFF0000) | delta;

	valpred = __builtin_brownie32_VALPRED(arg, valpred);
...
/**
	if ( valpred > 32767 )
	  ...
	  valpred = -32768;
**/
	valpred = __builtin_brownie32_CLAMPV(valpred);
