...
int adpcm_decoder(unsigned char* indata, int len) {
	...
	bufferstep = !bufferstep;
      ...
      /* if(bufferstep){inputbuffer = *indata++;}
      delta = (inputbuffer >> (4*bufferstep)) & 0xf;
      bufferstep = !bufferstep;
      */
	...
	// index += indexTable[delta];
	//index = __builtin_brownie32_STEP2(index);
	...
	if ( delta & 1 ) vpdiff += step>>2;
        //vpdiff = __builtin_brownie32_STEP4a(step,delta);

	if ( sign )
	  valpred -= vpdiff;
	else
	  valpred += vpdiff;
	//valpred = __builtin_brownie32_STEP4b(vpdiff,sign,valpred);
	...
	if ( valpred > 32767 )
	  ...
	  valpred = -32768;
	//valpred = __builtin_brownie32_STEP5(valpred);
